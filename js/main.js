...
if (!$journalEntry)
    throw new Error('$journalEntry did not query!');
if (!$entriesView)
    throw new Error('$entriesView did not query!');
if (!$navItem)
    throw new Error('$navItem did not query!');
if (!$newEntryButton)
    throw new Error('$newEntryButton did not query!');
if (!$titleInput)
    throw new Error('$titleInput did not query!');
if (!$notesInput)
    throw new Error('$notesInput did not query!');
if (!$mainHeading)
    throw new Error('$mainHeading did not query!');
if (!$titleForm)
    throw new Error('$titleForm did not query!');
if (!$deleteButton)
    throw new Error('$deleteButton did not query!');
if (!$confirmModal)
    throw new Error('$confirmModal did not query!');
...
$entryForm.addEventListener('submit', (event) => {
    ...
    if (data.editing) {
        ...
        const $oldListItem = $journalEntry.querySelector(`[data-entry-id='${data.editing.entryId}']`);
        if (!$oldListItem)
            throw new Error('$oldListItem did not query!');
...
$journalEntry.addEventListener('click', (event) => {
    ...
    if (!$parent)
        throw new Error('$entryForm closest() failed!');
    const $entryId = $parent.getAttribute('data-entry-id');
    if (!$entryId)
        throw new Error('$entryForm getAttribute() failed!');
    // search for and store that obj using the entryId to verify
    const entry = data.entries.find((entry) => entry.entryId.toString() === $entryId);
    if (!entry)
        throw new Error('data.entries find() failed!');
...
const toggleNoEntries = () => {
    ...
    if (!$noEntryText)
        throw new Error('$noEntryText failed to query!');
